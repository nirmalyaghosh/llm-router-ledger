...
- Added: `usage_details` on chat responses (`cost`, `is_byok`, `upstream_provider` where reported, plus `completion_tokens_details` / `prompt_tokens_details` flattened to `completion_*` / `prompt_*` keys, zero/null omitted), same split as embedding responses; `CostConfig.cache_write_input_per_1m`; `send_message()` gains a `messages` parameter for multi-turn conversations and tool loops, OpenAI content-parts shape, replacing `system` / `user` outright when supplied; `completion_tool_call_count` in `usage_details` on a turn that returned tool calls, so a tool-only turn (`response_length` 0) is distinguishable from an empty answer.
