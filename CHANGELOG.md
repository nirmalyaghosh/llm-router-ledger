...
- Changed: `send_message()` and `create_embeddings()` return a `ChatResult` / `EmbeddingResult` namedtuple; access as `.text` / `.vectors`, `.usage`, `.generation_id`. `CostConfig.estimate_cost()` takes `cached_tokens` / `cache_write_tokens` counts instead of a `cache_hit` bool, so a single call can mix cached, cache-write, and uncached input; raises `ValueError` if `cached_tokens + cache_write_tokens > input_tokens`. An explicit `0.0` rate is now honored as free rather than treated as unset. `UsageTracker` prompt/response previews are redacted by default (`preview_length` now defaults to 0); pass a positive `preview_length` to opt back in to stored previews.
